...
using Oceananigans.Grids, NCDatasets, Interpolations
...
# Extract latitude, longitude, and bathymetry depth from the dataset
lon = ds["lon"][:]  
lat = ds["lat"][:]  
depth = ds["elevation"][:, :]  # Bathymetry data (negative for ocean depth)

# Compute horizontal resolution
lat_res = abs(lat[2] - lat[1])  
lon_res = abs(lon[2] - lon[1])  

println("Latitude Resolution: ", lat_res, " degrees")
println("Longitude Resolution: ", lon_res, " degrees")

# Close the dataset
close(ds)

# Number of grid points in horizontal directions
Nφ = length(lat)  # Use dataset-defined points
Nλ = length(lon)

Nz = 25  # Number of vertical layers

arch = CPU()  # Change to GPU() if needed

# Define vertical grid (uniform, from ocean bottom to surface)
z_faces = range(minimum(depth), 0, length=Nz+1)  # Ensure negative depth at bottom
...
grid = LatitudeLongitudeGrid(
    ...
    z = z_faces  # Correct argument name
)

# Apply bathymetry as an immersed boundary
grid = ImmersedBoundaryGrid(grid, GridFittedBottom(depth))
...
function find_nearest_index(grid, lon, lat, z)
    ...
    z_vals   = collect(ugrid.z.cᵃᵃᶜ)  # correct depth values
...
lon_idx, lat_idx = find_nearest_index(grid, lon_release, lat_release, z)
...
# ugrid1 = grid.underlying_grid.z
# println("Available fields in ugrid: ", fieldnames(typeof(ugrid1)))
